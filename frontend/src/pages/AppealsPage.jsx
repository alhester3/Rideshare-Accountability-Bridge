    ...
    <div className="space-y-5">
      ...
      <section className={`rounded-2xl p-5 shadow-soft ${isAdmin ? "admin-glass" : "bg-white"}`}>
        <div className="flex flex-wrap gap-2 mb-3">
          {isAdmin && <>
            <select className="rounded-xl border p-2 text-slate-900" onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value }))}><option>All</option>{statuses.map((s) => <option key={s}>{s}</option>)}</select>
            <select className="rounded-xl border p-2 text-slate-900" onChange={(e) => setFilters((f) => ({ ...f, type: e.target.value }))}><option>All</option>{[...new Set(appeals.map((a) => a.type))].map((t) => <option key={t}>{t}</option>)}</select>
          </>}
        ...
        <div className="space-y-2">
          {visibleAppeals.map((appeal) => (
            <div key={appeal.id} className={`rounded-xl p-3 ${isAdmin ? "bg-slate-800" : "bg-slate-50"}`}>
              ...
              {isAdmin && <div className="mt-2 flex flex-wrap gap-2"><button onClick={() => setSelectedAppeal(appeal)} className="px-3 py-1 rounded-lg bg-electric text-white">Review</button><button onClick={() => updateAppealStatus(appeal.id, "escalate")} className="px-3 py-1 rounded-lg bg-amber-500 text-slate-900">Escalate</button><button onClick={() => updateAppealStatus(appeal.id, "approve")} className="px-3 py-1 rounded-lg bg-emerald-500 text-slate-900">Resolve</button></div>}
            </div>
          ))}
        </div>
      </section>
      ...
      {selectedAppeal && (
        <div className="fixed inset-0 bg-slate-900/50 grid place-items-center p-4">
          <div className="w-full max-w-xl rounded-2xl bg-white p-5 space-y-3">
            <h3 className="text-lg font-semibold">Appeal Review — {selectedAppeal.id}</h3>
            <p className="text-sm">{selectedAppeal.context}</p>
            <p className="text-sm">Original decision: {selectedAppeal.decisionId} · {decisions.find((d) => d.id === selectedAppeal.decisionId)?.summary}</p>
            <div className="flex flex-wrap gap-2"><button onClick={() => updateAppealStatus(selectedAppeal.id, "approve")} className="px-3 py-2 rounded-lg bg-emerald-500 text-white">Approve Appeal</button><button onClick={() => updateAppealStatus(selectedAppeal.id, "deny")} className="px-3 py-2 rounded-lg bg-rose-500 text-white">Deny Appeal</button><button onClick={() => updateAppealStatus(selectedAppeal.id, "escalate")} className="px-3 py-2 rounded-lg bg-amber-500">Escalate</button><button onClick={() => updateAppealStatus(selectedAppeal.id, "moreInfo")} className="px-3 py-2 rounded-lg border">Request More Info</button></div>
